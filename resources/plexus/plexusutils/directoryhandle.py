...
def addDir(name,url,mode,iconimage,total,pasta,fan_art="%s/fanart.jpg"%settings.getAddonInfo("path"),parser=None,parserfunction=None):
    ...
    if mode == 1 or mode == 2:
        ...
        if os.path.exists(os.path.join(mystrm_folder,fic)):
            contextmen.append(("Remove from my streams", 'XBMC.RunPlugin(%s?mode=12&url=%s&name=%s&iconimage=%s)' % (sysargv, urllib.quote_plus(url),name,iconimage)))
        else:
            contextmen.append(("Add to my streams", 'XBMC.RunPlugin(%s?mode=11&url=%s&name=%s&iconimage=%s)' % (sysargv,urllib.quote_plus(url),name,iconimage)))
