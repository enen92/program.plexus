...
stop_any_sop_traces(pid) -> Function to make use of all kill sopcast functions to be used in combination with p2p-pvr
    ...
    if not xbmc.getCondVisibility('system.platform.windows'):
        if xbmc.getCondVisibility('System.Platform.Android') or settings.getSetting('force_android') == "true":
            if  settings.getSetting('external-sopcast') == "0":
                ...
                if not pvr: sopstreams_builtin(name,iconimage,sop,False)
                else:
                    pid_sop,playable_url = sopstreams_builtin(name,iconimage,sop,True)
                    return pid_sop,playable_url
        else:
            if not pvr: sopstreams_builtin(name,iconimage,sop,False)
            else:
                pid_sop,playable_url = sopstreams_builtin(name,iconimage,sop,True)
...
def sopstreams_builtin(name,iconimage,sop,pvr=False):
    try:
        global spsc
        ...
        playable_url = "http://"+LOCAL_IP+":"+str(VIDEO_PORT)+"/"
        url=playable_url
